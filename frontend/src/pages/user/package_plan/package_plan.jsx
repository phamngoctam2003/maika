        ...
        PackageService.createVnpayPayment(id, returnUrl)
            .then(response => {
                ...
                if (response.data && response.success) {
                    ...
                    setShowPaymentLoading(false);
                    message.error(response.data?.message || "Không thể tạo thanh toán. Vui lòng thử lại sau.");
                }
            })
            .catch(error => {
                setShowPaymentLoading(false);
                ...
                if (error.response) {
                    ...
                    if (status === 400) {
                        AntNotification.showNotification('Không thể mua gói', data?.message || 'Có lỗi xảy ra khi tạo thanh toán.', 'warning');
                    } else if (status === 422) {
                        // Validation errors
                        const errorMessage = data?.message || 'Có lỗi xảy ra khi tạo thanh toán.';
                        AntNotification.showNotification('Chú ý', errorMessage, 'warning');
                    } else if (status === 429) {
                        AntNotification.showNotification('Quá nhiều yêu cầu', 'Vui lòng thử lại sau ít phút.', 'error');
                    } else if (status === 500) {
                        AntNotification.showNotification('Lỗi server', 'Server đang gặp sự cố. Vui lòng thử lại sau.', 'error');
                    } else {
                        AntNotification.showNotification('Không thể mua gói', data?.message || 'Đã xảy ra lỗi khi tạo thanh toán. Vui lòng thử lại sau.', 'warning');
                    }
                } else if (error.request) {
                    AntNotification.showNotification('Lỗi kết nối', 'Không thể kết nối đến server. Vui lòng kiểm tra internet.', 'error');
                } else {
                    AntNotification.showNotification('Lỗi không xác định', error.message || 'Đã xảy ra lỗi không xác định.', 'error');
                }
            });
    }
        ...
        <div className="bg-gradient-to-br w-full">
            ...
            <div className='lg:h-[259px] h-[200px] w-full bg-member-package-plan relative lg:text-left text-center'>
                ...
                <div className="absolute bottom-0 left-0 z-[5] px-4 lg:px-12 w-full lg:py-10 py-6">
                    <p className="font-bold text-4xl-50-55 text-white-50 text-2xl"> Gói hội viên </p>
            ...
            <div className="mx-auto px-4 lg:px-12 w-full lg:py-6 py-4 bg-gradient-to-br from-gray-900 via-gray-800 to-green-900 min-h-screen">
                <div className="mb-8 md:mb-12">
                    ...
                    <p className="text-gray-300 text-sm md:text-base">
                        Sử dụng tài khoản ngân hàng, ví điện tử hoặc thẻ thanh toán quốc tế (Visa, Master) thông qua VNPay
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                    {packages.map((plan, index) => (
                        <div
                            key={index}
                            className={`relative bg-gray-800 rounded-2xl p-6 md:p-8 border-2 transition-all duration-300 ${plan.is_best_offer
                        ...
                        >
                            ...
                            <div className="text-center mb-6 mt-4">
                                ...
                                <button
                                    ...
                                    className={`w-full font-bold py-3 px-6 rounded-xl transition-colors duration-300 text-sm md:text-base ${plan.is_purchased
                ...
                <div className="mt-8 md:mt-12 text-center">
                    <p className="text-gray-400 text-xs md:text-sm mb-2">
                        * Giá đã bao gồm VAT. Thanh toán an toàn qua VNPay - Bảo mật SSL 256-bit
                    </p>
                    <div className="bg-yellow-900/30 border border-yellow-600/50 rounded-lg p-4 mt-4">
                        ...
                        <ul className="text-yellow-300 text-xs md:text-sm space-y-1 text-left">
                            <li>• Bạn có <strong>60 phút</strong> để hoàn thành thanh toán sau khi nhấn "MUA GÓI"</li>
                            <li>• Không đóng trình duyệt hoặc tab thanh toán trong quá trình giao dịch</li>
