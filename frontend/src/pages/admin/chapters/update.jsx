...
const Update_Chapter = () => {
    ...
    const handSubmit = async (e) => {
        ...
        try {
            ...
            if (res) {
                AntNotification.showNotification("Cập nhật thành công", "Chương đã được cập nhật thành công!" || res.message, "success");
                navigate("/admin/books/chapters/" + chapter.book_format_mapping.book_id + "?type=" + (chapter.book_format_mapping.format_id === 2 ? "audio" : "ebook"));
            } else {
                AntNotification.showNotification("Có lỗi xảy ra", "Cập nhật chương thất bại!" || res.message, "error");
            }
        } catch (error) {
            AntNotification.handleError(error);
        }
                ...
                <form className="max-w-lg mt-5" onSubmit={handSubmit}>
                    ...
                    <div className="mb-5">
                        ...
                        <input type="number"
                            ...
                            className="shadow-sm border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 w-full"
                            onChange={e => setChapter({ ...chapter, chapter_order: e.target.value })}
                        />
                    </div>
                    {/* Hiển thị phần nhập nội dung nếu là ebook */}
                    {
                        chapter?.book_format_mapping?.format_id === 1 && (
                            <div className="mb-5">
                                ...
                                <QuillEditor onChange={handleEditorChange} dataEditor={chapter?.content} />
                            </div>
                        )
                    }
