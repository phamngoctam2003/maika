...
const Book_Types = () => {
    ...
    const breadcrumbItems = [
        ...
        { label: "Quản Lý Loại Sách", path: null },
    ];

    const checkBooktype = (e, id) => {
        setSelectedBooktype((prevSelected) => {
            if (e.target.checked) {
    ...
    const hanDleDelete = async () => {
        ...
        try {
            const res = await BooktypeService.destroy(selectedBooktype);
            if (res?.status === 200) {
                setSelectedBooktype([]);
                AntNotification.showNotification(
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await BooktypeService.getBookTypes({
                page: currentPage,
                per_page: pageSize,
            ...
            if (res) {
                setBooktype(Array.isArray(res.data) ? res.data : []);
            ...
            <div className="relative overflow-x-auto shadow-md my-4 sm:rounded-lg bg-white">
                ...
                <div className="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-2 px-4 bg-white">
                    ...
                    <div className="py-1 flex flex-wrap-reverse">
                        {(selectedBooktype.length > 0) ?
                            <DeleteConfirmationModal
                                data={`Bạn có chắc chắn muốn xóa ${selectedBooktype.length} loại sách này không?`}
                        ...
                        <tr>
                            <th scope="col" className="p-4">
                                <div className="flex items-center">
                                    <input
                                        checked={selectedBooktype.length === booktypes.length}
                                        onChange={() => {
                                            if (selectedBooktype.length === booktypes.length) {
                                                setSelectedBooktype([]); // bo chon tat ca
                                            } else {
                                                setSelectedBooktype(
                                                    booktypes.map((booktype) => booktype.id)
                                                ); // chon tat ca
                                            }
                                        }}
                            ...
                            booktypes.length === 0 ? (
                                ...
                                >
                                    <td className="w-4 p-4">
                                        <div className="flex items-center">
                                            <input
                                                ...
                                                onChange={(e) => checkBooktype(e, booktype.id)}
                                                checked={selectedBooktype.includes(booktype.id)}
                                                type="checkbox"
                                                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                            />
                                            <label
                                    ...
                                    <td className="px-6 py-4 text-gray-900 dark:text-slate-950 font-semibold">
                                        {booktype.category ? booktype.category.name : "Không có danh mục"}
                                    </td>
                                    <td className="px-6 py-4">
                                        {new Date(booktype.created_at).toLocaleDateString("vi-VN", {
                                            year: "numeric",
                                            month: "2-digit",
                                            day: "2-digit",
                                        })}
                                    ...
                                    <td className="px-6 py-4">
                                        <Link
                                            to={`/admin/book-types/update/${booktype.id}`}
                                            type="button"
