...
const UpdatePackage = () => {
    ...
    const breadcrumbItems = [
        ...
        { label: "Cập Nhật Gói Cước", path: null },
    ];
            ...
            try {
                ...
                if (res) {
                    ...
                    setFormValues({
                        ...
                        highlight_label: res.highlight_label || "",
                        is_best_offer: !!res.is_best_offer,
                    });
                } else {
                    AntNotification.showNotification("Có lỗi xảy ra", "Không thể tải dữ liệu gói cước!", "error");
                }
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await PackageService.update(id, formValues);
            if (res) {
                AntNotification.showNotification("Cập nhật thành công", "Gói cước đã được cập nhật!", "success");
                navigate("/admin/packages");
            } else {
                AntNotification.showNotification("Có lỗi xảy ra", "Cập nhật gói cước thất bại!", "error");
            }
        } catch (error) {
            AntNotification.handleError(error);
        }
                ...
                <form className="max-w-lg mt-5" onSubmit={handleSubmit}>
                    ...
                    <div className="mb-5">
                        ...
                        <input
                            ...
                            className="shadow-sm border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 w-full"
                            value={packageData?.original_price === formValues.discounted_price ? '' : formValues.discounted_price}
                            onChange={handleInputChange}
                        />
                    </div>
                    <div className="mb-5">
                    ...
                    <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Cập nhật gói cước</button>
                </form>
